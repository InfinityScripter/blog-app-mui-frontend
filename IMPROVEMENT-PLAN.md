...
B4 security hardening; B5 Telegram CTAs; B7 polish. Rate-limit XFF-spoof bypass
found by review and FIXED (getTrustedClientIp + allowlist).

**B6 public /llm-stats dashboard — БОЛЬШЕ НЕ СУЩЕСТВУЕТ, и это решение, а не регресс.**
Страницу `/[locale]/llm-stats` и публичную ручку `GET /api/llm-stats/public` собрали
2026-07-11 и в тот же день сняли по решению владельца («не хочу светить свою
статистику»): FE 7a4500b, BE b907f5e, в проде 404. Осталась только приватная
админка `/dashboard/admin/llm-stats` за авторизацией плюс локальный сканер
`npm run llm-stats:push`. **Не отстраивать заново «по плану»** — личная статистика
использования AI приватна намеренно. Проверено 2026-07-26: папки `llm-stats` во
фронте нет.

DELIBERATELY SCOPED (claims corrected to match reality):

- **B3 frontend pagination — ДОДЕЛАНО 2026-07-26** (коммит `524940e`). Оговорка «acceptable
  at current scale (~8 posts)» истекла: постов стало 146, и главная вшивала в payload
  все, показывая десять — 531 КБ HTML+RSC (85 КБ gzip), рост ~2 поста в день без
  потолка. Фид переведён на `?page`/`&limit` через `useSWRInfinite`, первая страница
  по-прежнему приходит из SSR готовым HTML для краулеров. Замерено на проде: **280 КБ
  сырых / 41 КБ gzip при 10 вшитых постах**, следующие страницы по ~17 КБ по требованию.
  Проверено живьём: «Показать ещё» доливает 11→21 пост; полный обход корпуса сохранён
  (sitemap 510 URL, llms.txt 88 КБ, RSS все 6 блоговых постов); ряд фильтров
  по-прежнему показывает все 32 тега корпуса, а не 8 из первой страницы — словарь
  считает сервер.
