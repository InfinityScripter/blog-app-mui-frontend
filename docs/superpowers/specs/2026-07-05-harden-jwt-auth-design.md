...
- The access-token JWT payload is unchanged (`{ userId, role }`), so
  ...
  the _transport_ changes (cookie instead of `Authorization` header).
...
3. **Found but `revoked_at` set** → this is a _reused, already-rotated_ token →
...
- Routes:
  ...
  - `google/callback.ts`, `yandex/callback.ts` — **stop putting the token in the
    ...
    calls `/me`.
  - `google.ts`, `yandex.ts` (OAuth _start_) — unchanged.
...
6. **Theft:** stolen refresh token used after the real client already rotated →
   ...
   re-login required. Stolen _access_ token dies within 15 min regardless.
...
- **Deploy backend first** (it accepts _both_ cookie and legacy bearer during the
...
- No moving off JWT to opaque server sessions for the _access_ token (refresh is
